...
class ChildTool(BaseTool):
    ...
    # NOTE: BaseTool的invoke最后调用的也是run
    def invoke(
        self,
    ...
    def _parse_input(
        ...
        if isinstance(tool_input, str):
            ...
            if input_args is not None:
                # NOTE: 这儿进行了参数的校验，要求调用函数时候的参数要和FN意图中中配置的参数一致，从而限制了方法的灵活性
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # NOTE: 从这里看，只要传入到tool_input里，就可以实现差异的参数化设置
        ...
        # NOTE: BaseTool里工具执行失败的是，会写入到observation里，然后再次扔给大模型处理
        # 由于缺少了信息，agent会问用户，再加上是多轮对话，用户的信息又会被回归进去从而实现了参数的补全
...
"""
StructuredTool(name="", description=函数名(参数)一直到开头的注释部分, args_schema=""...)
v2：不再携参数
"""
    ...
    def _make_with_name(tool_name: str) -> Callable:
        def _make_tool(dec_func: Union[Callable, Runnable]) -> BaseTool:
            if isinstance(dec_func, Runnable):
                ...
                # NOTE: repr会将对象转化为供解释器读取的形式
                # core.tool修饰的对象,会带有参数的
            ...
            # NOTE: 可以看到，在使用schema参数的时候，返回的是一个StructuredTool对象
            ...
            # NOTE: 调用tool修饰符生成的结果是一个标准形式的tool形式
            return Tool(
                name=tool_name,
                func=func,
...
# NOTE: render_text_description只是将tool的名称和描述拼接成一个字符串
