...
class TextSplitter(BaseDocumentTransformer, ABC):
    ...
    def create_documents(
        ...
        for i, text in enumerate(texts):
            index = -1
            # NOTE: 将每个text作为一个单元来构建文档，这说明这里的text原本就应该对应一个文档中的内容
            # 如果我们是切分好的一个文档内容，就不需要这个切分的过程了
            for chunk in self.split_text(text):
                metadata = copy.deepcopy(_metadatas[i])
                if self._add_start_index:
                    index = text.find(chunk, index + 1)
                    metadata["start_index"] = index
                # NOTE: 将每个chunk作为一个子文档进行构建，只是这里的metadata里会存他属于那个文档
                # 因此，documents是将所有的文档的内容存在一起的，只通过metadata信息来判别当前的Document属于哪个文档
    ...
    def split_documents(self, documents: Iterable[Document]) -> List[Document]:
        """Split documents."""
        texts, metadatas = [], []
        # NOTE: 这里documents指得就是多个文档
        for doc in documents:
            texts.append(doc.page_content)
            metadatas.append(doc.metadata)
        # NOTE: 切分文档是将文档中的内容先出来，然后再去调用create_documents做文档的text_splitter
